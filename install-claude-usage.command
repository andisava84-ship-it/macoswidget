#!/bin/zsh
# Claude Usage — one-click installer. Double-click in Finder to run.
# Builds a tiny menu-bar widget that shows your real Claude plan usage.
set -u
cd "$HOME" 2>/dev/null || true
clear
echo "──────────────────────────────────────────────"
echo "   Claude Usage  ·  menu-bar widget installer"
echo "──────────────────────────────────────────────"
echo

# Self-clear quarantine if this file was AirDropped/downloaded (harmless if absent).
xattr -d com.apple.quarantine "$0" 2>/dev/null || true

# 1) Need the Swift compiler (Xcode Command Line Tools).
if ! xcode-select -p >/dev/null 2>&1 || ! /usr/bin/xcrun --find swiftc >/dev/null 2>&1; then
  echo "One-time setup: Xcode Command Line Tools are required."
  echo "A macOS dialog will open — click \"Install\" and wait for it to finish,"
...
mkdir -p "$DIR/Sources" "$APP/Contents/MacOS"

echo "→ Writing source…"
cat > "$DIR/Sources/main.swift" <<'SWIFT_EOF'
...
final class AppDelegate: NSObject, NSApplicationDelegate, NSMenuDelegate {
    ...
    func updateButton() {
        ...
        switch latest {
        case .ok(let u):
            let f5 = (u.fiveHour?.pct ?? 0) / 100
            btn.image = segmentImage(frac: f5, color: gaugeColor(f5))
            btn.title = " \(pctInt(u.fiveHour?.pct ?? 0))%·\(pctInt(u.sevenDay?.pct ?? 0))%"
        case .needsLogin:
...
echo "→ Compiling (~20s)…"
if ! swiftc -O -o "$APP/Contents/MacOS/ClaudeUsage" "$DIR/Sources/main.swift" \
       -framework Cocoa -target "$(uname -m)-apple-macosx13.0"; then
  echo
  echo "❌ Build failed. If Xcode Command Line Tools just finished installing,"
...
<plist version="1.0"><dict>
  ...
  <key>CFBundleIdentifier</key><string>com.claudeusagebar</string>
...
if ! security find-generic-password -s "Claude Code-credentials" >/dev/null 2>&1; then
  echo "⚠️  You're not signed into Claude Code yet, so the widget will show \"login\"."
  echo "    Fix: run  claude  in a terminal, type  /login , then click the widget's \"Refresh now\"."
else
  echo "• If macOS asks to use the \"Claude Code-credentials\" Keychain item → click Always Allow."
  echo "• The widget is now in your menu bar (top-right): dots + 5h%·weekly%. Click it for details."
  echo "• Click the icon → \"Launch at login\" to keep it running after restarts."
fi
echo
